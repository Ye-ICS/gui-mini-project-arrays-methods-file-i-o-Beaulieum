    ...
    public void start(Stage stage) {
        ...
        for(int i = 0; i < buttonAmount.length; i++){
            ...
            buttonIndex[1] = i;
                ...
                boolean endScreenChecker = Logic.checkIfComplete(buttonValue);
                if(endScreenChecker = false){
    ...
    private void changeButtons(Button[][] buttonAmount, int[] index, int[][] buttonValue) {
        ...
        for(int i = 0; i < buttonAmount.length; i++){
            if(buttonValue[k][i] == 0){
                buttonAmount[k][i].setStyle("-fx-background-color: white;-fx-border-color: black;-fx-border-width: 1;");
            }else if(buttonValue[k][i] == 1){
                buttonAmount[k][i].setStyle("-fx-background-color: red;-fx-border-color: black;-fx-border-width: 1;");
            }else if(buttonValue[k][i] == 1){
                buttonAmount[k][i].setStyle("-fx-background-color: garnet;-fx-border-color: black;-fx-border-width: 1;");
            }
